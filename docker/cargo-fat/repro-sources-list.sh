...
"debian")
	...
	# NOTE (RP2026 local patch): buster and older use the pre-bullseye security
	# URL layout where the Release file lives under <codename>/updates/, not
	# <codename>-security/. Bullseye and newer use the modern layout.
	...
	"buster"|"stretch"|"jessie")
		echo "deb [check-valid-until=no] ${SNAPSHOT_ARCHIVE_BASE}debian-security/${snapshot} ${VERSION_CODENAME}/updates main" >>/etc/apt/sources.list
		;;
	*)
