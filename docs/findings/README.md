...
| [`failure-class-fixability.md`](failure-class-fixability.md) | Class-by-class analysis of the 1,249 DS1 `not_reproducible` candidates: what's pipeline-recoverable (nightly variant +77, build-tool augment +24 certain; 250 RUSTC_BITROT carry a toolchain-bitrot signature — error fires in a locked transitive dep — prior-milestone retry plausibly +50–100) vs provably corpus property (~500+: author-env tests, git-dep rot, missing system libs). Bounds the ceiling at low-to-mid 60s %. |
