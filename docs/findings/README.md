...
| [`recovery-experiments.md`](recovery-experiments.md) | **Every proposed recovery lever, actually rebuilt** (22 candidates, 9 buckets). Confirmed wins: nasm build-tool augmentation (2/2), E0308 prior-milestone (2/2). Instructive failures: current-nightly clears the Rocket abort but is too new (E0119 in traitobject); E0512/E0793/E0283/llvm/relax-locked/git-dep-gone don't recover. Confirmed recoverable ≈ +85 (nasm ~24 + E0308 ~61); rest is corpus property. |
| [`failure-class-fixability.md`](failure-class-fixability.md) | Class-by-class analysis of the 1,249 DS1 `not_reproducible` candidates: what's pipeline-recoverable (nightly variant +77, build-tool augment +24 certain; RUSTC_BITROT prior-milestone retry — **verified by a 10-candidate local rebuild**: E0308 stdlib-evolution codes recover 2/2, E0119/E0034 coherence codes do not 0/7, so the recoverable pool is ~88 not 250) vs provably corpus property (~600+: author-env tests, git-dep rot, missing system libs, stable coherence bitrot). Bounds the ceiling at ~60 %. |
| [`taxonomy-literature.md`](taxonomy-literature.md) | Modern (2020–2026) failure-taxonomy literature scan, primary-source-verified. Positions DURP's two schemes: Scheme 1 keeps BUMP as anchor (+ Breaking-Good's "indirect compilation failure" subcategory, Kong 2024 npm as complementary cause-taxonomy); Scheme 2's closest precedent is Fu et al. 2026 (embedded build-reconstruction, 5 coarse cats) which DURP extends; the corpus-vs-fixable axis is novel. Zheng 2025 (GHA, 16 cats, κ 0.708) is the modern κ benchmark + κ target. |
