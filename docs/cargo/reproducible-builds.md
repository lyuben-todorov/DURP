...
| Risk | Mitigation |
| --- | --- |
| `snapshot.debian.org` eviction for old dates | Snapshot coverage is stable for stretch / buster / bullseye / bookworm within our current bucketing window. `repro-sources-list.sh` handles the pre-bullseye security-URL layout (buster/stretch/jessie) via its case statement. Stretch was added to the grid 2026-05-12 (paired with rustc 1.39) for pre-2019 candidates; stretch's libssl 1.0.2 + 1.1.0 dual-shipping is the right libssl-dev for `openssl-sys 0.9.x`-era crates. Stretch builds require `--allow-unauthenticated` on the apt install because some snapshot security keys were rotated before the keyring shipping in `rust:1.39-stretch` saw them; see the OpenSSL retry experiment in [`../findings/ds1-full-findings.md`](../findings/ds1-full-findings.md). |
